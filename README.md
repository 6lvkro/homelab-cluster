...
**4-Tier 직렬 의존 홈랩 인프라 선언적 관리 매니페스트 레포지토리**

> 고가용성 ARM SBC 2대 (control-plane/IoT, storage) + x86_64 데스크탑 1대 (NVIDIA GPU/CI) 구성 기준 템플릿
>
> 노드 수와 역할은 [`.env.config`](.env.example)에서 조정하며, 하나의 노드가 여러 역할을 가질 수 있다.
>
> 환경 값은 `${VAR}` 플레이스홀더로만 두고, 비민감 설정은 [`.env.config`](.env.example), 시크릿은 [`Infisical`](2-platform/helm-releases.yaml#L193)에서 배포 시점에 주입한다.
>
> 이 레포는 로컬 Gitea 서버에서 CI 워크플로우([`mirror-public`](.gitea/workflows/mirror-public.yaml))를 통해 개인 설정을 제외한 상태로 Github에 미러링된다.
...
1-data/                # Tier 1: 데이터베이스 (PG + VectorChord, Redis)
...
├── ci/                # Gitea, Act Runner
├── storage/           # Samba
└── helm-releases.yaml # Loki, Alloy, Velero, Infisical, Secrets Operator

3-apps/                # Tier 3: End User 워크로드
├── ai/                # e.g. Ollama, Open-WebUI, ComfyUI, TTS, STT, ...
├── iot/               # e.g. Home Assistant, MQTT, Z2M, ...
├── media/             # e.g. Immich, Jellyfin, *arr, ...
├── tools/             # e.g. code-server, pod-cleanup, 생산성, ...
└── ...                # 기타 카테고리별 앱
