...
| :-------------- | :------------------------------------------------------------------------------------ |
| `/`             | `DEFAULT_PASSWORD`                                                                    |
| `/postgres`     | `DB_PASSWORD_POSTGRES` 및 `init-databases.sh`가 생성하는 DB별 `DB_PASSWORD_<SERVICE>` |
...
| `/pocket-id`    | `POCKET_ID_ENCRYPTION_KEY` 및 OIDC 연동 서비스별 `POCKET_ID_OIDC_<SERVICE>`           |
...
> 초기 생성 이후 `.env.config` 변경은 CI 워크플로우로 자동화 한다. ([`sync-config.yaml`](.gitea/workflows/sync-config.yaml))
