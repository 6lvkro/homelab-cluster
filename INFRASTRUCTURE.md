...
PG + VectorChord (pgvector/pgvecto.rs 확장 포함), Redis
...
| [`sqlite-backup`](components/sqlite-backup/)                   | Deployment | `sqlite-backup` 라벨 Deployment에 sqlite3 번들 initContainer와 Velero pre-hook 주입 |
...
Flux HelmRelease로 선언적 관리된다. values는 HelmRelease에 inline 정의하되,
Alloy 설정처럼 분량이 큰 것은 `configMapGenerator`로 분리하고 차트가 이름으로 참조한다.
...
| `grafana/alloy`                              | default        | [`2-platform/helm-releases.yaml`](2-platform/helm-releases.yaml) |
...
Ingress로 노출되는 서비스는 원칙적으로 Pocket ID + OAuth2-Proxy를 통해 인증된다.

| 인증 패턴                | 적용 방식                                                    | 예시                        |
| :----------------------- | :----------------------------------------------------------- | :-------------------------- |
| **ForwardAuth**          | `components/oauth-ingress/`가 자동 주입. 앱 수정 불필요      | 대부분의 서비스             |
| **OIDC**                 | 앱이 Pocket ID에 직접 OIDC 연동. Ingress에 `auth: self` 라벨 | Grafana, Open-WebUI, Immich |
| **자체 인증 유지**       | 앱 기능에 인증이 묶여있는 경우. Ingress에 `auth: self` 라벨  | Home Assistant, Jellyfin    |
| **인증 없음 (LAN 전용)** | hostNetwork 또는 내부 전용 서비스                            | MQTT 브로커                 |

> Helm 차트가 자체 생성하는 Ingress는 kustomize 컴포넌트가 닿지 않아 ForwardAuth가 주입되지 않는다.
> 현재 Infisical이 여기 해당하며 앱 자체 로그인에만 의존한다.
...
Grafana Alloy를 DaemonSet으로 배포해 각 노드의 Pod 로그를 수집하고 Loki로 전송한다.
보존 기간 설정은 7일이다.

Alloy 설정은 [`alloy/config.alloy`](2-platform/observability/alloy/config.alloy), Grafana의 Loki 데이터소스는 [`grafana/datasources.yaml`](2-platform/observability/grafana/datasources.yaml)로 프로비저닝된다.
